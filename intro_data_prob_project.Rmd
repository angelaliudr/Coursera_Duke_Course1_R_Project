...
Fruits and vegetables provide essential nutrients to main human health. It is anticipated that people with higher fruit and vegetable intake will have better health. 
...
Variable `X_frutsum` is a calculated quantity, reflecting the number of fruits consumed per day. The value of this variable has two implicit decimal places, so we will be dividing its value by 100 during the analysis. 

Similarly, variable `X_vegesum` is a calculated quantity, describing the number of vegetables consumed per day. Again, the value has two implied decimal places, and we will divide this value by 100 before other calcualations. 
...
In the above plot, the X-axis lists the five health categories and the Y-axis is the mean of the total amount of fruit and vegetable consumed per day for people within that health category. The vertical lines represent the standard deviations of the total amount of fruit and vegetable consumption. From this plot, we can see that there is a general trend that healthier people have higher fruit and vegetable intake. However, the large standard deviations suggest that the different health categories do not have significant differences. 
...
For question 1, we found healthier individuals tend to consume more fruit and vegetables per day. But the statistics are not strong enough to differentiate the different health categories. 

For question 2, we found that the physical activity level is correlated with the health status: individuals with high physical activity levels tend to self identify themselves as having good or better health whereas people with low physical activity tend to identify themselves as having poorer health. For question 3, we found that people with existing medical conditions tend to have worse health status, from obesity, high cholesterol, arthitis, asthma, to diabetes, in increasing severity of the impact. Using Binomials distributions as approximation, we found these differences of particular subject groups from the property of the general sample to be statistically signicant. 
